...
export default function SettingsPage() {
  ...
  useEffect(() => {
    getMyShop().then((s) => {
      setShop(s); setName(s.name||""); setAddress(s.address||""); setPhoneDisplay(s.phone_display||""); setGreeting(s.greeting||""); setHours(s.business_hours||{});
  ...
  async function handleSave() {
    ...
    try {
      const updated = await updateShop({ name, address, phone_display: phoneDisplay, greeting, services: servicesList, business_hours: hours });
      setShop(updated); setSaved(true); setTimeout(() => setSaved(false), 3000);
    } catch (err: unknown) {
