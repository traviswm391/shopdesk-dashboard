...
export default function SettingsPage() {
  ...
  async function handleSave() {
        ...
        setError("");
        setSaved(false);
        const servicesList = services.filter((s) => s.name.trim()).map((s) =>
                s.price ? `${s.name.trim()} $${s.price.trim()}` : s.name.trim()
                                                                           );
        try {
                const payload = { name, address, phone_display: phoneDisplay, greeting, services: servicesList, business_hours: hours };
  ...
  if (loading) return <div className="flex items-center justify-center h-screen"><div className="w-8 h-8 border-2 border-orange-500 border-t-transparent rounded-full animate-spin" /></div>div>;
  
    return (
          <div className="p-8 max-w-2xl">
                <div className="mb-8">
                        <h1 className="text-2xl font-bold text-white">Settings</h1>h1>
                        <p className="text-gray-400 text-sm mt-0.5">Update your shop info — changes sync to your AI receptionist automatically.</p>p>
                </div>div>
            {!shop && !error && (
            ...
            {error && <div className="bg-red-500/10 border border-red-500/30 rounded-lg px-4 py-3 mb-6 text-red-400 text-sm">{error}</div>div>}
            {saved && <div className="bg-green-500/10 border border-green-500/30 rounded-lg px-4 py-3 mb-6 text-green-400 text-sm">Saved! Your AI receptionist has been updated.</div>div>}
                <div className="bg-gray-800 border border-gray-700 rounded-2xl p-6 mb-5">
                        <h2 className="text-base font-semibold text-white mb-5">Shop Info</h2>h2>
                        <div className="space-y-4">
                          {[
            { label: "Shop Name", value: name, setter: setName, placeholder: "Mike's Auto Repair" },
            ...
            { label: "AI Opening Greeting", value: greeting, setter: setGreeting, placeholder: `Thanks for calling ${name||"the shop"}, what can we do for you?` },
                                    ...
                                    <div key={label}>
                                                  <label className="block text-sm font-medium text-gray-300 mb-1.5">{label}</label>label>
                ...
                <div className="bg-gray-800 border border-gray-700 rounded-2xl p-6 mb-5">
                        <h2 className="text-base font-semibold text-white mb-5">Business Hours</h2>h2>
                        <div className="space-y-3">
                          {DAYS.map((day) => {
                        const h = hours[day] || { open: "9:00 AM", close: "5:00 PM", closed: false };
                        return (
                                        <div key={day} className="flex items-center gap-3">
                                                        <div className="w-24 text-sm font-medium text-gray-300">{DAY_LABELS[day]}</div>div>
                                                        <div onClick={() => updateHours(day, "closed", !h.closed)} className={`w-10 h-5 rounded-full transition cursor-pointer ${h.closed ? "bg-gray-600" : "bg-orange-500"} relative flex items-center`}>
                                                                          <div className={`w-4 h-4 bg-white rounded-full shadow transition-transform absolute ${h.closed ? "translate-x-0.5" : "translate-x-5"}`} />
                                                        </div>div>
                                                        <span className="text-sm text-gray-400 w-12">{h.closed ? "Closed" : "Open"}</span>span>
                                          {!h.closed && (<>
                                                            <select value={h.open} onChange={(e) => updateHours(day, "open", e.target.value)} className="flex-1 bg-gray-900 border border-gray-600 rounded-lg px-3 py-1.5 text-white text-sm focus:outline-none focus:border-orange-500">
                                                              {TIME_OPTIONS.map((t) => <option key={t}>{t}</option>option>)}
                                                            </select>select>
                                                            <span className="text-gray-500 text-sm">to</span>span>
                ...
                <div className="bg-gray-800 border border-gray-700 rounded-2xl p-6 mb-6">
                        <h2 className="text-base font-semibold text-white mb-5">Services</h2>h2>
                        <div className="space-y-3">
                          {services.map((s, i) => (
                        <div key={i} className="flex gap-2">
                                      <input value={s.name} onChange={(e) => setServices((prev) => prev.map((x,j) => j===i?{...x,name:e.target.value}:x))} placeholder="e.g. Oil Change" className="flex-1 bg-gray-900 border border-gray-600 rounded-lg px-4 py-2.5 text-white placeholder-gray-500 focus:outline-none focus:border-orange-500 text-sm transition" />
                                      <div className="relative">
                                                      <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm">$</span>span>
                          ...
                          {services.length > 1 && <button onClick={() => setServices((prev) => prev.filter((_,j) => j!==i))} className="p-2.5 text-gray-500 hover:text-red-400 transition">x</button>button>}
                        </div>
                      ))}
                ...
                <button onClick={handleSave} disabled={saving} className="w-full bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-white font-semibold rounded-xl px-4 py-3.5 transition">
                  {saving ? "Saving & updating AI..." : "Save Changes"}
                </button>button>
            {shop?.phone_number && (
                    <div className="mt-6 bg-gray-800 border border-gray-700 rounded-2xl p-5">
                              <p className="text-xs text-gray-400 uppercase tracking-wider mb-1">Your AI Phone Number</p>p>
                              <p className="text-xl font-bold text-white">{shop.phone_number}</p>p>
                    </div>div>
                )}
          </div>div>
        );
